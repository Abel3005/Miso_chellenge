...
class PoseTransformer(object):
    ...
    def __init__(self, class_type):
        ...
        miso_ts_pth = 'datasets/miso_big/MISOTS'
        miso_big_pth = 'datasets/miso_big/MISOBIG'
...
class Basic_Utils():
    ...
    def __init__(self, config):
        ...
        self.miso_ts_cls_ptsxyz_dict = {}
        self.miso_ts_cls_ptsxyz_cuda_dict = {}
        self.miso_ts_cls_kps_dict = {}
        self.miso_ts_cls_ctr_dict = {}
        self.miso_big_cls_ptsxyz_dict = {}
        self.miso_big_cls_ptsxyz_cuda_dict = {}
        self.miso_big_cls_kps_dict = {}
        self.miso_big_cls_ctr_dict = {}
    ...
    def get_cls_name(self, cls, ds_type):
        if type(cls) is int:
            ...
            elif ds_type == 'miso_ts':
                cls = self.miso_ts_cls_lst[cls -1]
            elif ds_type == 'miso_big':
                cls = self.miso_big_cls_lst[cls -1]
            else:
    ...
    def get_pointxyz(
        ...
        if ds_type == "ycb":
            ...
            ptxyz_pth = os.path.join(
                'datasets/miso_ts/Miso_TS_preprocessed/models',
                'obj_%02d.ply' % cls
            )
            ...
            self.miso_ts_cls_ptsxyz_dict[cls] = pointxyz
            return pointxyz
        elif ds_type == "miso_big":
            ptxyz_pth = os.path.join(
                'datasets/miso_big/Miso_BIG_preprocessed/models',
                'obj_%02d.ply' % cls
            )
            pointxyz = self.ply_vtx(ptxyz_pth) / 1000.0
            dellist = [j for j in range(0, len(pointxyz))]
            dellist = random.sample(dellist, len(pointxyz) - 2000)
            pointxyz = np.delete(pointxyz, dellist, axis=0)
            self.miso_big_cls_ptsxyz_dict[cls] = pointxyz
            return pointxyz
        else:
            ptxyz_pth = os.path.join(
    ...
    def get_pointxyz_cuda(
        ...
        if ds_type == "ycb":
            ...
            if cls in self.miso_ts_cls_ptsxyz_cuda_dict.keys():
                return self.miso_ts_cls_ptsxyz_cuda_dict[cls].clone()
            ptsxyz = self.get_pointxyz(cls, ds_type)
            ptsxyz_cu = torch.from_numpy(ptsxyz.astype(np.float32)).cuda()
            self.miso_ts_cls_ptsxyz_cuda_dict[cls] = ptsxyz_cu
            return ptsxyz_cu.clone()
        elif ds_type == "miso_big":
            if cls in self.miso_big_cls_ptsxyz_cuda_dict.keys():
                return self.miso_big_cls_ptsxyz_cuda_dict[cls].clone()
            ptsxyz = self.get_pointxyz(cls, ds_type)
            ptsxyz_cu = torch.from_numpy(ptsxyz.astype(np.float32)).cuda()
            self.miso_big_cls_ptsxyz_cuda_dict[cls] = ptsxyz_cu
            return ptsxyz_cu.clone()
        else:
    ...
    def get_kps(
        ...
        if type(cls) is int:
            if ds_type == 'ycb':
                ...
                cls = self.config.miso_ts_id2obj_dict[cls]
            elif ds_type == 'miso_big':
                cls = self.config.miso_big_id2obj_dict[cls]
            else:
        ...
        if ds_type == "ycb":
            ...
            if cls in self.miso_ts_cls_kps_dict.keys():
                return self.miso_ts_cls_kps_dict[cls].copy()
            if use_orbfps:
                kps_pth = self.config.kp_orbfps_ptn % (cls, self.config.n_keypoints)
            else:
                kps_pattern = os.path.join(
                    self.config.miso_ts_kps_dir, "{}/{}.txt".format(cls, kp_type)
                )
                kps_pth = kps_pattern.format(cls)
            print("kps_pth in get_kps:", kps_pth)
            kps = np.loadtxt(kps_pth, dtype=np.float32)
            self.miso_ts_cls_kps_dict[cls] = kps
        elif ds_type == "miso_big":
            if cls in self.miso_big_cls_kps_dict.keys():
                return self.miso_big_cls_kps_dict[cls].copy()
            if use_orbfps:
                kps_pth = self.config.kp_orbfps_ptn % (cls, self.config.n_keypoints)
            else:
                kps_pattern = os.path.join(
                    self.config.miso_big_kps_dir, "{}/{}.txt".format(cls, kp_type)
                )
                kps_pth = kps_pattern.format(cls)
            print("kps_pth in get_kps:", kps_pth)
            kps = np.loadtxt(kps_pth, dtype=np.float32)
            self.miso_big_cls_kps_dict[cls] = kps
        else:
    ...
    def get_ctr(self, cls, ds_type='ycb', ctr_pth=None):
        ...
        if ds_type == "ycb":
            ...
            if cls in self.miso_ts_cls_ctr_dict.keys():
                return self.miso_ts_cls_ctr_dict[cls].copy()
            cor_pattern = os.path.join(
                self.config.kp_orbfps_dir, '{}_corners.txt'.format(cls),
            )
            cors = np.loadtxt(cor_pattern.format(cls), dtype=np.float32)
            ctr = cors.mean(0)
            self.miso_ts_cls_ctr_dict[cls] = ctr
        elif ds_type == "miso_big":
            if cls in self.miso_big_cls_ctr_dict.keys():
                return self.miso_big_cls_ctr_dict[cls].copy()
            cor_pattern = os.path.join(
                self.config.kp_orbfps_dir, '{}_corners.txt'.format(cls),
            )
            cors = np.loadtxt(cor_pattern.format(cls), dtype=np.float32)
            ctr = cors.mean(0)
            self.miso_big_cls_ctr_dict[cls] = ctr
        else:

...
#criterion : segment compare
#criterion_of : key, center point compare
def model_fn_decorator(
    ...
    def model_fn(
        ...
        with torch.set_grad_enabled(not is_eval):
            ...
            loss_lst = [
                (loss_rgb_seg, 2.0), (loss_kp_of, 1.0), (loss_ctr_of, 1.0),
            ]
            ...
            if args.debug:
                show_lb = view_labels(
                    data['rgb'], data['cld_rgb_nrm'][0, :3, :], cls_rgb
                )
            ...
            loss_dict = {
                'loss_rgb_seg': loss_rgb_seg.item(),
            ...
            if is_test and test_pose:
                ...
                if not args.test_gt:
                    ...
                    teval.eval_pose_parallel(
                        cld, cu_dt['rgb'], cls_rgb, end_points['pred_ctr_ofs'],
...
def train():
    ...
    trainer = Trainer(
        ...
        model_fn,
        optimizer,
        checkpoint_name=os.path.join(checkpoint_fd, "PXVN_%s" % args.cls),
        best_name=os.path.join(checkpoint_fd, "PXVN_%s_best" % args.cls),
